...
| `/supercompact [here] [keep N]`         | Interactive slash command    | Yes (tool calls and reasoning deleted)        | Copies and switches by default; `here` rewrites the current session file                    | Archive artifact for `here` only; a copied run leaves the original as the record     |
...
deletes every tool call together with its result, plus every reasoning block.
User and assistant messages survive verbatim from the first turn.
...
while breaking the pairing.

`anthropicServerTool` blocks are out of scope. They are stored verbatim so
same-provider replay can re-send them unchanged.

### Where the removed data goes

`/supercompact` calls `AgentSession.fork()` first, so the original session file
keeps every byte and is itself the complete record of what was removed. Nothing
else is needed, and nothing else is written.

A requested copy that does not happen aborts the operation. `fork()` also returns
false when an extension cancelled `session_before_switch`, and answering "do not
switch sessions" by rewriting the current one is the opposite of what was asked.

`/supercompact here` rewrites the current session, which is the only way to reduce
a session that cannot be copied. That run has no original to fall back on, so it
writes every removed call, result and reasoning block to one session artifact
first and refuses to proceed if that write fails.

Either path is rejected while a response is streaming, because it rewrites the
history the live turn is reading from.

### Keeping recent rounds

`/supercompact keep 3` leaves the last three rounds whole.
`compaction.supercompactKeepRecentTurns` (default `0`) sets the default.

The count is rounds, not tokens: the walk stops at the start of the Nth-from-last
`user` or `bashExecution` message, so a kept window can never separate a tool
result from the call that produced it. Only a real turn counts. Compaction's
`findTurnStartIndex` also treats `custom_message` and `branch_summary` as turn
starts, which is right for cutting a summary boundary but wrong here: an injected
reminder would become the boundary and the round the setting promised to keep
would be reduced anyway.

### As an automatic compaction method

`supercompact` is selectable in `compaction.methodOrder`, though not in the
default order. Automatic maintenance runs it through the same inline path as
shake (`SessionMaintenance.#runInlineReduction`), so it inherits the dead-loop
guard from issues #2119/#2275, the provider-anchored recovery band, and the
continuation scheduling. Shake's own failure semantics are unchanged.

The automatic path never copies the session, because a compaction method must not
change session identity mid-turn. It therefore always writes the archive.

### Provider replay

Messages that lost blocks drop `providerPayload`. A native-history copy of an
assistant turn would otherwise replay the original blocks and undo the operation,
the same reason `provider-image-budget.ts` clears it when it clamps image
content. Provider sessions that cache message identity are closed through
`closeCodexProviderSessionsForHistoryRewrite()`, and the displayed context budget
is corrected via `recordAnchoredHistoryRewrite()`, measured by counting the
entries inside the billed prompt before and after rather than estimating.

### Implementation

- [`../packages/agent/src/compaction/supercompact.ts`](../packages/agent/src/compaction/supercompact.ts) — pure region detection and in-place deletion.
- `SessionMaintenance.supercompactContext()` — archive, persistence, context replay.
- `SessionMaintenance.#runInlineReduction()` — shared automatic path for `shake` and `supercompact`.
- `AgentSession.supercompact()` — copy, then reduce.
- [`../packages/coding-agent/src/session/supercompact-types.ts`](../packages/coding-agent/src/session/supercompact-types.ts) — options, result shape and operator summary.
